...
class TabOne(wx.Panel):
    def __init__(self, parent):
        ...
        self.panel2 = TabOne_panel_two(self)

        
        #5 x 5 grid sizer
        sizer = wx.GridBagSizer(5,5)
        sizer.Add(dateText, pos=(3, 0)) #Date static text nested on left
        sizer.Add(self.tc1, pos=(3, 1), span=(1, 3), flag=wx.TOP|wx.EXPAND)#text box for entering date next to static text
        sizer.Add(fileLocText, pos=(4, 0), flag=wx.LEFT|wx.TOP)#file input sizer static text
        sizer.Add(self.tc2, pos=(4, 1), span=(1, 3), flag=wx.TOP|wx.EXPAND)#tc2 input text box
        sizer.Add(inputButton, pos=(4, 4), flag=wx.TOP|wx.RIGHT)# file input button sizer
        sizer.Add(self.tc3, pos=(5, 1), span=(1, 3),flag=wx.TOP|wx.EXPAND)
        sizer.Add(SaveLocText, pos=(5, 0), flag=wx.TOP|wx.LEFT)
        sizer.Add(outputButton, pos=(5, 4), flag=wx.TOP|wx.RIGHT)#brow button save
        sizer.Add(self.radio1, pos=(7,1))#radio Mortgage bestex
        sizer.Add(self.radio2, pos=(7,2))#mtg best ex button
        sizer.Add(self.radio3, pos=(7,3))
        sizer.Add(helpButton, pos=(8, 0), flag=wx.LEFT)#help button
        sizer.Add(runButton, pos=(8, 3))
        sizer.Add(self.panel2, pos=(1,1.5))# Run
        self.SetSizer(sizer)
...
class TabOne_panel_two(TabOne):
    def __init__( self, tc1=None ):
        super(TabOne,self).__init__(tc1)
        wx.Panel.__init__( self, parent='TabOne')
        self.runMtg = wx.Button(self, label="Run Mortgage Best Ex Previous Business Day")
        self.runMtg.Bind(wx.EVT_BUTTON, self.runPreviousBdayMtg)
        
        self.runTreas = wx.Button(self, label="Run Treasury Best Ex Previous Business Day")
        self.runTreas.Bind(wx.EVT_BUTTON, self.runPreviousBdayTreas)
        
        
        sizer = wx.GridBagSizer(1, 4)
        sizer.Add(self.runMtg, pos=(1,1))# Run
        sizer.Add(self.runTreas, pos=(2,1))
        self.SetSizer(sizer)
...
class TabTwo(wx.Panel):
    ...
    def lowPrice(self, event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            rpt.save()
            try:
                rpt2 = executedOrderReport(orderReports[1], save, int(priceThresh),int(advThres))
                rpt2.save()
            except IndexError:
                pass
